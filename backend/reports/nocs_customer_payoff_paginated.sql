-- Get Customer Wise Payoff Balance by NOCS (PAGINATED - OPTIMIZED)
-- Parameters: :nocs_code (required), :offset (pagination offset), :limit (rows per page)
-- Orders by payoff balance (highest to lowest)
-- Performance: Uses ROW_NUMBER for efficient pagination
...
customer_with_names AS (
    SELECT
        ...
        cb.CUSTOMER_TYPE,
        cb.PAYOFF_BALANCE,
        ROW_NUMBER() OVER (ORDER BY cb.PAYOFF_BALANCE DESC) AS row_num
