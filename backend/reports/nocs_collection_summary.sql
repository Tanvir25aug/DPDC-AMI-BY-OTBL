...
SELECT
    ...
    ROUND(SUM(t1.tender_amt), 2) AS PRINCIPAL_AMOUNT,
    ROUND(SUM(t1.tender_amt) * 0.05, 2) AS VAT_AMOUNT,
    ROUND(SUM(t1.tender_amt) * 1.05, 2) AS TOTAL_AMOUNT,
    COUNT(DISTINCT t1.pay_event_id) AS TRANSACTION_COUNT,
    TO_CHAR(MIN(t2.pay_dt), 'DD-MON-YYYY') AS FIRST_PAYMENT_DATE,
    TO_CHAR(MAX(t2.pay_dt), 'DD-MON-YYYY') AS LAST_PAYMENT_DATE
FROM CI_PAY_TNDR T1
...
GROUP BY p.CHAR_VAL, l.descr
ORDER BY SUM(t1.tender_amt) DESC;
