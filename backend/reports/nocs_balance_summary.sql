...
SELECT /*+ PARALLEL(4) */
    ...
    -- DISCONNECT ELIGIBLE: Customers with total_balance > 200 (owe more than 200 BDT)
    COUNT(CASE WHEN nocs_balances.total_balance > 200 THEN 1 END)                         AS DISCONNECT_ELIGIBLE_QTY,
    SUM(CASE WHEN nocs_balances.total_balance > 200 THEN nocs_balances.total_balance ELSE 0 END) AS DISCONNECT_ELIGIBLE_AMT
FROM (
