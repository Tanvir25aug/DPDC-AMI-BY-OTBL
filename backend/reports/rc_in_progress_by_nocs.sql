-- RC In Progress - NOCS Breakdown
-- Returns count by NOCS for chart/analysis

SELECT
    vl.descr AS NOCS_NAME,
    COUNT(*) AS COUNT,
    SUM(CASE WHEN (SYSDATE - l.START_DTTM) * 24 >= 2 THEN 1 ELSE 0 END) AS STUCK_COUNT,
    ROUND(AVG((SYSDATE - l.START_DTTM) * 24 * 60), 2) AS AVG_MINUTES
FROM ci_acct a
...
INNER JOIN d1_activity PARTITION({{PARTITION}}) l ON l.D1_ACTIVITY_ID = k.D1_ACTIVITY_ID
